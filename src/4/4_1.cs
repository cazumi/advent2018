    ...
    class four_one
    {
        ...
        public static void TaskFour(){
           ...
           for(int i=0; i<guardsList.Length; i++){
               ...
               dateString = guardAttribute[0].Substring(1);
               var timeString = guardAttribute[1].Remove(guardAttribute[1].Length - 1);
               var sleep = 0;
               var awake = 0;
               var guardStatus = "";
               ...
               if(!guards.Any()){
                  createGuard(id, dateString);
               }
               else{
                   foreach(var g in guards)
                   {
                       if(g.id != id || g.date != dateString){
                           createGuard(id, dateString);
                       }else{
